...
<div class="data_list">
	...
	<div class="commentDatas">
		<c:choose>
			...
			<c:otherwise>
				<!--varStatus="status" 属性相当于实现了对行数的绑定处理-->
				<c:forEach var="comment" items="${commentList }" varStatus="status">
						<c:choose>
							<c:when test="${status.index<10 }">
								<div class="comment">
									<span><font>${status.index+1 }楼&nbsp;&nbsp;&nbsp;&nbsp;
											${comment.userIp }：</font>${comment.content }&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										[&nbsp;<fmt:formatDate value="${comment.commentDate }" type="date" pattern="yyyy-MM-dd HH:mm"/>&nbsp;]</span>
								</div>								
							</c:when>
							<c:otherwise>
...
<div class="data_list" >
	...
	<div class="publish_comment">
			<div>
				<textarea style="width: 100%" rows="3" id="content" name="content" placeholder="少侠有何高见..."></textarea>
			</div>
			<div class="verCode">
				<!--防止用户刷屏-->
				验证码：<input type="text" value="${imageCode }" name="imageCode"  id="imageCode" size="10"
						   onkeydown= "if(event.keyCode==13)form1.submit()"/>&nbsp;
				<img onclick="javascript:loadimage();" title="看不清，换一张试试" name="randImage" id="randImage"
